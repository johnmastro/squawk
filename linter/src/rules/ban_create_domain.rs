        ...
        match &raw_stmt.stmt {
            Stmt::CreateDomainStmt(stmt) if stmt.constraints.len() > 0 => {
                ...
                errs.push(RuleViolation::new(
                    RuleViolationKind::BanCreateDomain,
                    raw_stmt.into(),
                    None,
                ));
            }
...
mod test_rules {
    ...
    fn lint_sql(sql: &str) -> Vec<RuleViolation> {
        check_sql_with_rule(sql, &RuleViolationKind::BanCreateDomain, None, false).unwrap()
    }

    #[test]
    fn ban_create_domain_no_false_positives() {
        let sql = r#"
